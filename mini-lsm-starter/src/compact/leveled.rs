...
use crate::compact::SimpleLeveledCompactionTask;
    ...
    pub fn generate_compaction_task(
        ...
        for (_, files) in &snapshot.levels {
            level_sizes.push(
                files
                    .iter()
                    .map(|i| snapshot.sstables.get(i).unwrap().table_size())
                    .sum::<u64>() as usize,
            );
        }
        let base_level_size_bytes = self.options.base_level_size_mb * 1024 * 1024;
        target_level_size[self.options.max_levels - 1] =
            level_sizes[self.options.max_levels - 1].max(base_level_size_bytes);
        ...
        // 触发 L0 与 base level 的压缩
        // base level 之前的 level 的期望大小都是 0，所以可以直接刷写到 base level
        ...
        for level in 0..self.options.max_levels {
            let prio = level_sizes[level] as f64 / target_level_size[level] as f64;
            if prio > 1.0 {
        ...
        // 最后一个 level 最大就 1.0，所以不用考虑最后一个 level 的 case
