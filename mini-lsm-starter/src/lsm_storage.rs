...
impl MiniLsm {
    pub fn close(&self) -> Result<()> {
        let mut mutex = self.flush_thread.lock();
        mutex.take().unwrap().join().ok();
        Ok(())
    }
...
impl LsmStorageInner {
    ...
    pub fn get(&self, key: &[u8]) -> Result<Option<Bytes>> {
        ...
        let mut memtable = snapshot.memtable.clone();
        let mut iter = snapshot.imm_memtables.iter();
        loop {
            ...
            if let Some(next_memtable) = iter.next() {
                memtable = next_memtable.clone();
                continue;
            }
        ...
        if l0_iter.is_valid() && l0_iter.key().raw_ref() == key {
            if l0_iter.value().is_empty() {
                return Ok(None);
            }
        ...
        for (_, level_sst_ids) in &snapshot.levels {
            ...
            for tid in level_sst_ids {
                let table = snapshot.sstables[tid].clone();
                if keep_table(key, &table) {
            ...
            if level_iter.is_valid() && level_iter.key().raw_ref() == key {
    ...
    pub fn force_freeze_memtable(&self, _state_lock_observer: &MutexGuard<'_, ()>) -> Result<()> {
        ...
        let memtable = MemTable::create(sst_id);
        {
            let mut guard = self.state.write();
            let mut snapshot = guard.as_ref().clone();
            let prev = std::mem::replace(&mut snapshot.memtable, Arc::new(memtable));
            snapshot.imm_memtables.insert(0, prev.clone());
    ...
    pub fn force_flush_next_imm_memtable(&self) -> Result<()> {
        ...
        let imm_to_flush = {
            ...
            guard
                .imm_memtables
                .last()
                .ok_or(anyhow!("imm memtable list is empty"))?
                .clone()
        };
        let mut builder = SsTableBuilder::new(self.options.block_size);
        imm_to_flush.flush(&mut builder)?;
        let id = imm_to_flush.id();
        let sst =
            Arc::new(builder.build(id, Some(self.block_cache.clone()), self.path_of_sst(id))?);
        {
            let mut guard = self.state.write();
            let mut snapshot = guard.deref().deref().clone();
            ...
            if self.compaction_controller.flush_to_l0() {
                ...
                // tiered compaction
                // 直接开新层
                // You may use the first output SST id as the level/tier id for your new sorted run.
    ...
    pub fn scan(
        ...
        let iter = snapshot.imm_memtables.iter();
        let memtable = snapshot.memtable.clone();
        let mut vec = Vec::new();
        vec.push(Box::new(memtable.scan(lower, upper)));
        for table in iter {
            vec.push(Box::new(table.scan(lower, upper)));
        }
        let mem_iter = MergeIterator::create(vec);
        let ss_iter = snapshot.l0_sstables.iter();
        let mut vec = Vec::new();
        for table_id in ss_iter {
            ...
            if range_overlap(
                ...
                vec.push(Box::new(iter));
            }
        }
        let ss_iter = MergeIterator::create(vec);

        // leveled
        let mut level_iters = Vec::with_capacity(snapshot.levels.len());

        for (i, v) in &snapshot.levels {
            let level_ssts = v
                .to_vec()
                .iter()
                .map(|tid| snapshot.sstables[tid].clone())
                .filter(|table| {
                    range_overlap(
                        lower,
                        upper,
        ...
        LsmIterator::new(
            TwoMergeIterator::create(
                TwoMergeIterator::create(mem_iter, ss_iter)?,
                MergeIterator::create(level_iters),
            )?,
            map_bound(upper),
        )
        .map(|iter| FusedIterator::new(iter))
    }
}
